...
export default function Navbar({ toggleSidebar }: NavbarProps) {
  ...
  return (
    <header className="z-20 border-b border-slate-200/80 bg-white/80 backdrop-blur-xl dark:border-slate-800/50 dark:bg-slate-950/40 flex h-16 shrink-0 items-center justify-between px-6 transition-all duration-300">
      <div className="flex items-center gap-4">
        {toggleSidebar && (
          <button 
            onClick={toggleSidebar}
            className="rounded-xl p-2 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-white/5 dark:hover:text-slate-200 md:hidden"
            type="button"
          >
        ...
        <div>
          <h2 className="text-lg font-bold tracking-tight text-slate-900 dark:text-white">{getPageTitle()}</h2>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        <div className="hidden md:flex items-center relative">
          ...
          <button
            ...
            className="glass-input w-72 rounded-full py-2 pl-9 pr-4 text-sm text-slate-500 text-left transition-all hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-slate-950/55 flex items-center justify-between cursor-pointer"
          >
            <span>Search...</span>
        ...
        <div className="flex items-center gap-3 relative">
          ...
          <button 
            ...
            className="glass-pill relative rounded-full p-2 text-slate-500 transition-all hover:scale-105 hover:text-slate-700 dark:text-slate-350 dark:hover:text-white" 
            type="button" 
        ...
        <div className="relative">
          <button 
            ...
            className="glass-pill flex items-center gap-2 rounded-full p-1.5 pr-2 transition-all hover:scale-[1.02] hover:bg-slate-50 dark:hover:bg-white/15"
            type="button"
            aria-haspopup="menu"
          ...
          >
            ...
            <div className="hidden md:block text-left">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 capitalize">{plan} Plan</p>
            </div>
          ...
          {dropdownOpen && (
            <>
              ...
              <div className="glass-popover absolute right-0 z-50 mt-3 w-52 overflow-hidden py-1" role="menu">
                ...
                <button 
                  onClick={logout}
                  className="w-full flex items-center gap-2 px-4 py-2 text-sm text-red-655 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors cursor-pointer"
                >
